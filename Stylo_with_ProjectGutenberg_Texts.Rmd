...
In this notebook we will work will build build corpora out of texts from Project Gutenberg in roder to try out computational stylistics, also known as "stylometry.". Corpus preparation and file structure for the stylo package is quite specific and this notebook makes that task easier. 
...
We need to load the stylo package from a GitHub account (at the time of writing this notebook it was not available in CRAN). If you are running iOS you will need to download https://www.xquartz.org/ and restart your system. You may get a note to update some files.  Make sure you have the console below open so that you can agree to those updates. 
